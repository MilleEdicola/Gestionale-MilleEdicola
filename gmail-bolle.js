...
function FlussoDiario() {
  ...
  const caricaDatiGiorno = useCallback(async () => {
    try {
      const res = await fetch('/api/dati-giorno?data=' + dataLavoro)
      const json = await res.json()
      setBolleDelGiorno(json.bolle || [])
      setVenditeDelGiorno(json.vendite || [])
      setArchivioTestate(json.testate || [])
  ...
  const confermaBolla = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/salva-bolla', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ bolla, righe })
      })
      const json = await res.json()
  ...
  const registraVendita = async (testata, categoria, prezzo, quantita, bc) => {
    ...
    const res = await fetch('/api/salva-vendita', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ vendita })
    })
    const json = await res.json()
    if (json.error) { flash('Errore: ' + json.error, false); return }
    setVenditeDelGiorno(prev => [vendita, ...prev])
